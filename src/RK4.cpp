...
Particle::Particle(int charge, int mass, arma::vec position, arma::vec velocity)
{
    c = charge;
    m = mass;
    r = position;
...
vec forces(vector<Particle>& particles, int index, double t)
{
    ...
    {   
        if (i != index)
        {
            ...
            tot_force += 10*particles[i].c * R/(pow(arma::norm(R), 3));
        }
    }    
...
//Forward Euler ment for comparing to RK4
